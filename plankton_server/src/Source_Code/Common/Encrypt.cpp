...
static int ip_1_table[64] = { 40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55,
		23, 63, 31, 38, 6, 46, 14, 54, 22, 62, 30, 37, 5, 45, 13, 53, 21, 61,
		29, 36, 4, 44, 12, 52, 20, 60, 28, 35, 3, 43, 11, 51, 19, 59, 27, 34, 2,
		42, 10, 50, 18, 58, 26, 33, 1, 41, 9, 49, 17, 57, 25 };

static int e_table[48] = { 32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8, 9, 10, 11,
		12, 13, 12, 13, 14, 15, 16, 17, 16, 17, 18, 19, 20, 21, 20, 21, 22, 23,
		24, 25, 24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1 };

static int p_table[32] = { 16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5, 18,
		31, 10, 2, 8, 24, 14, 32, 27, 3, 9, 19, 13, 30, 6, 22, 11, 4, 25 };

static int s_box_table[8][4][16] = { { { 14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6,
		12, 5, 9, 0, 7 },
...
//对输入的字节串作BCD编码扩展
int Encrypt::ByteToBCD(unsigned char* bytes, int count, unsigned char* strBCD) {
	unsigned char cTmp;
	int i;
...
//把输入的BCD编码串还原成字节串
int Encrypt::BCDToByte(unsigned char* strBCD, int count, unsigned char* bytes) {
	unsigned char cTmp;
	int i;
...
//DES加密/解密
int Encrypt::Do_DES(char* strSrc, char* strKey, char* strDest, char flag) {
	unsigned char subKey[16][48 + 1], byte8[8 + 1], bits[64 + 1],
			strTmp[64 + 1];
...
string Encrypt::encrypt(string encryptContent) {
	if (encryptContent.empty() || strlen(key) == 0) {
		return "";
	}

	char result[1024];
	char resourse[this->caculateEncryptLength(encryptContent) * 16 + 1];
	memset(resourse, 0, sizeof(resourse));
	memset(result, 0, sizeof(result));
	strcpy(resourse, Converter::string_to_hex(encryptContent).c_str());
	this->Do_DES(resourse, key, result, 'e');
	return string(result);
}

//解密
string Encrypt::decrypt(string decryptContent) {
	if (decryptContent.empty() || strlen(key) == 0) {
		return "";
	}

	char result[1024];
	char resourse[this->caculateEncryptLength(decryptContent) * 16 + 1];
	memset(resourse, 0, sizeof(resourse));
	memset(result, 0, sizeof(result));
	strcpy(resourse, decryptContent.c_str());
	this->Do_DES(resourse, key, result, 'd');
	return Converter::hex_to_string(string(result));
}

//返回加密容器长度
int Encrypt::caculateEncryptLength(string content) {
	int contentLengthFactor = content.length() / 16;
